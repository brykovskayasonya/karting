...
// MVP «Апекс»: заезд можно оценить, если бронь активна (не отменена) и заезд уже прошел.
fun BookingDetailsState.canRate(clock: AppClock): Boolean {
    val booking = currentBooking ?: return false
    val slotStartAt = booking.slot?.startAt ?: return false
    return booking.status == BookingStatus.Active && slotStartAt <= clock.now()
}
